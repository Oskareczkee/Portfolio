...
private:
	...
	Node* insert(Node* rootNode, std::pair<Key, T> content)
	{
	...
	Node* deleteNode(Node* rootNode, Key key)
	{
		if (rootNode == nullptr)
			return nullptr;
	...
	Node* find(Node* rootNode, const Key& key)
	{
		...
		if (rootNode->key == key)
			...
			rootNode = find(rootNode->left, key);
		else if (key > rootNode->key)
			rootNode = find(rootNode->right, key);
...
public:
	...
	void insert(std::pair<Key, T> val)
	{
